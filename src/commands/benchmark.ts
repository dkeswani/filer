...
function detectScopes(root: string): string[] {
  ...
  // First preference: extract top-level directories from actual node scopes
  if (index && index.nodes.length > 0) {
    for (const node of index.nodes) {
      for (const scope of node.scope) {
        // Take first 2 path segments: "frontend/src" or "backend/app"
        const parts = scope.split('/').slice(0, 2).join('/');
        if (parts && !parts.startsWith('.')) dirs.add(parts);
      }
    }
  }

  // Fallback: extract from config include patterns
  if (dirs.size === 0 && config) {
