...
function printNode(node: AnyNode): void {
  ...
  switch (node.type) {
    case 'intent':
      ...
      if (node.owns.length)         printList('Owns:', node.owns);
      if (node.does_not_own.length) printList('Does NOT own:', node.does_not_own, chalk.dim);
      break;
    ...
    case 'constraint':
      ...
      if (node.instead) console.log('  ' + chalk.green('Instead:') + ' ' + node.instead);
      break;

    case 'assumption':
      console.log('  ' + chalk.bold('Statement:')    + ' ' + node.statement);
      console.log('  ' + chalk.red('Breaks when:')   + ' ' + node.breaks_when);
      if (node.boundary) console.log('  ' + chalk.green('Boundary:') + ' ' + node.boundary);
      if (node.relied_on_by.length) printList('Relied on by:', node.relied_on_by);
      break;
    ...
    case 'danger':
      ...
      if (node.current_mitigation) console.log('  ' + chalk.dim('Mitigation: ') + node.current_mitigation);
      if (node.history)            console.log('  ' + chalk.dim('History: ')    + node.history);
      break;

    case 'pattern':
      console.log('  ' + chalk.bold('Statement:') + ' ' + node.statement);
      console.log('  ' + chalk.bold('Why:')       + ' ' + node.why);
      if (node.structure)     console.log('  ' + chalk.dim('Structure: ')    + node.structure);
      if (node.anti_pattern)  console.log('  ' + chalk.red('Anti-pattern: ') + node.anti_pattern);
    ...
    case 'decision':
      ...
      if (node.revisit_if) console.log('  ' + chalk.dim('Revisit if: ') + node.revisit_if);
    ...
    case 'security':
      ...
      if (node.audit_required) {
        ...
        if (node.audit_format) console.log('  ' + chalk.dim('Format: ') + node.audit_format);
      }
      break;
...
function getTypeColor(type: NodeType): (s: string) => string {
  const colors: Record<NodeType, (s: string) => string> = {
    security:    chalk.red,
...
function formatConfidence(c: number): string {
  ...
  if (pct >= 95) return chalk.green(`${pct}%`);
  if (pct >= 80) return chalk.yellow(`${pct}%`);
  return chalk.red(`${pct}%`);
}

function printList(label: string, items: string[], colorFn?: (s: string) => string): void {
