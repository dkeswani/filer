...
program
  ...
  .option('--smart-compress',            'AST-aware compression — strips comments via tree-sitter (~70% token reduction)')
  ...
  .option('--root <path>',               'Pack a local directory (default: cwd)')
  ...
  .option('--no-file-summary',           'Omit the top-files summary section from output')
  .option('--no-directory-structure',    'Omit the directory tree section from output')
