...
import ora from 'ora';
import { scanFiles, groupIntoModules, getChangedFiles, getCurrentCommit, } from './scanner.js';
import { extractNodes } from './extractor.js';
import { ensureFilerDirs, upsertNode, writeIndex, buildIndex, readAllNodes, readConfig, } from '../store/mod.js';
    ...
    for (let i = 0; i < modules.length; i++) {
        ...
        try {
            ...
            for (const node of extraction.nodes) {
                ...
                const typeConfig = config.node_types?.[node.type];
...
export async function runUpdate(root, opts = {}) {
    ...
    if (changed.length === 0) {
        if (!opts.silent)
            console.log(chalk.dim('  No changed files since last commit.'));
        return { modules_processed: 0, nodes_created: 0, nodes_updated: 0, nodes_rejected: 0, files_indexed: 0, estimated_usd: 0, errors: [] };
    }
    if (!opts.silent) {
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function printModuleList(modules, log) {
